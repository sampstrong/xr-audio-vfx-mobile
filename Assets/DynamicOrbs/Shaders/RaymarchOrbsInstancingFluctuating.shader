Shader "Raymarch/OrbsInstancingFluctuating"
{
    Properties
    {
    	[Header(Main Colors)]
    	...
    	_FresnelIntensity ("Fresnel Intensity", Range(0, 10)) = 0.85
        _FresnelRamp ("Fresnel Ramp", Range(0, 10)) = 0.77
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 _FresnelIntensity, _FresnelRamp;
            ...
            float gyroid(float3 p, float3 wp)
            {
	            ...
	            p *= r;
            	//float thickness = (sin(_Time.y * 0.5) * 0.5 + 0.5) * _GyroidThickness; // dynamic
            	float thickness = getThickness();
            	// float thickness = _GyroidThickness; // static
            ...
            float fres(float3 n, float3 rd)
            {
                ...
                f = pow(f, _FresnelRamp) * _FresnelIntensity;
