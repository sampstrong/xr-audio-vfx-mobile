Shader "Raymarch/OrbsInstancingHaze"
{
    Properties
    {
    	[Header(Main Colors)]
    	...
    	[Header(Haze)]
    	[Space]
    	_HazeColor ("Haze Color", Color) = (0.6, 0.6, 0.6, 1.0)
    	_HazeMaxDist ("Haze Max Distance", Range(10, 500)) = 65.0
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                if (d.w < MAX_DIST)
                {
                	...
                	// haze
                	float camDist = distance(p, ro);
                	camDist /= _HazeMaxDist;
                	col = lerp(col, _HazeColor, camDist);
                }
                else
                {
	                 discard;
