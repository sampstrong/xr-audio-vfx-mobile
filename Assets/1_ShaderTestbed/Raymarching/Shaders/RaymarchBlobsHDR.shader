Shader "SamStrong/RaymarchBlobsHDR"
{
    Properties
    {
        _MainTex ("Texture", 2D) = "white" {}
    	_SmoothAmount ("Smooth Amount", Range(0, 0.2)) = 0.1
    	_GlowAmount ("Glow Amount", Float) = 1.0
    }
    SubShader
    {
        ...
        Pass
        {
            ...
            sampler2D _MainTex;
            float4 _MainTex_ST;
            float _SmoothAmount, _GlowAmount;
            ...
            float sphere1(float3 p)
            {
            	...
            	float radius = (sin(_Time.y) * 0.3 + 0.7) * 0.17;
            ...
            {
				float yOffset = cos(_Time.y * 0.1) * 0.15;
            	float xOffset = sin(_Time.y * 0.2) * 0.15;
            	float zOffset = cos(_Time.y * 0.5) * 0.2;
            	float3 offset = float3(xOffset, yOffset, zOffset);

            	p += offset;
            	
            	float radius = (cos(_Time.y) * 0.2 + 0.7) * 0.2;
	            float d = length(p) - radius;
            ...
            {
				float yOffset = cos(_Time.y * 0.15) * 0.08;
            ...
            {
				float yOffset = sin(_Time.y * 0.1) * 0.1;
            	float xOffset = sin(_Time.y * 0.2) * 0.2;
            ...
            float sphere5(float3 p)
            {
            	...
            	float xOffset = cos(_Time.y * 0.6) * 0.2;
